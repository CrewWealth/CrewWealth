...
class TestPolishTemplateFeatures(unittest.TestCase):
    ...
    def test_dashboard_has_onboarding_tour_button_and_hooks(self):
        ...
        self.assertIn("id=\"tourRestartButton\"", content)
    ...
    def test_guide_route_exists(self):
        content = MAIN_ROUTES.read_text(encoding="utf-8")
        self.assertIn("@main_bp.route('/guide')", content)
        self.assertIn("return render_template('guide.html')", content)
